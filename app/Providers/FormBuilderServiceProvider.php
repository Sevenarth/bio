...
class FormBuilderServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
      ...
      Blade::directive('formTextfield', function ($arguments) {
          ...
          <label for="'.$name.'"><?php echo '.$plaintext.' ? "<b>":""?><?php echo __("'.$title.'") ?><?php echo '.$plaintext.' ? "<b>":""?><?php echo ('.$required.') ? "" : " <small class=\"text-muted\">".__("(optional)")."</small>"; ?></label>
          <div class="input-group">
          <?php if(!empty("'.$prepend.'")) { ?>
        <div class="input-group-prepend">
          <div class="input-group-text">'.$prepend.'</div>
        </div><?php } ?>
          <input type="'.$type.'" <?php echo ('.$plaintext.' ? "readonly" : "") ?> value="<?php if($old = old("'.$name.'")) echo $old; '. ($editMode ? 'elseif(isset($'.$editMode.')){ echo $'.$editMode.'->'.$name.'; }' : '').'else{ ?>'.$value.'<?php } ?>" class="<?php echo (!empty("'.$class.'")) ? "'.$class.' " : "form-control" . ('.$plaintext.' ? "-plaintext" : ""); ?><?php echo ($errors->has("'.$name.'")) ? " is-invalid" : ""; ?>" name="'.$name.'" id="'.$name.'" placeholder="<?php echo __("'.$placeholder.'") ?>"<?php echo ('.$required.') ? \' required\' : \'\'; ?>>
          <?php if(!empty("'.$append.'")) { ?>
          ...
          <?php if(!empty("'.$description.'")) { ?>
            <small class="text-block text-muted"><?php echo __("'.$description.'"); ?></small><?php } ?>
          </fieldset>';
      });
